...
if test $CODE -ne 0; then
 echo "Git fetch from origin \"$GIT_BRANCH_DST\" error!"; exit 102
fi
git -C repository checkout $GIT_BRANCH_DST; CODE=$?
if test $CODE -ne 0; then
 echo "Git checkout to \"$GIT_BRANCH_DST\" error!"; exit 103
fi
git -C repository merge --no-ff --no-commit $GIT_COMMIT_SRC; CODE=$?
if test $CODE -ne 0; then
 echo "Git merge with ${GIT_COMMIT_SRC::7} failed!"; exit 104
fi
