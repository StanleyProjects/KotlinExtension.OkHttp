...
fun OkHttpClient.execute(url: String): Response {
    return newCall(url = url).execute()
}
...
fun OkHttpClient.execute(
    ...
    return newCall(url = url, query = query, header = header, method = method, body = body).execute()
}
